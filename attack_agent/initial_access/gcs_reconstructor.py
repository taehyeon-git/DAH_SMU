...
def reconstruct_gcs(doc: IntelDocument) -> IntelDocument:
    ...
    doc.gcs_model = model
    doc.candidate_actions = build_candidate_actions(model)
    doc.recommended_chain = [action.action_id for action in doc.candidate_actions[:4]]
    doc.confidence = 0.8 if current else 0.6
    doc.observations.append({"type": "gcs_reconstruction", "weak_point_count": len(model.weak_points)})
    return doc


def build_candidate_actions(model: GCSModel) -> list[CandidateAction]:
    actions = [
        CandidateAction(
            action_id="ACT-JAMMER-001",
            agent="dah-jammer",
            action_type="EW_LINK_DEGRADATION_SIM",
            reason="Router/TMMR link metrics influence dashboard communication state.",
        ...
        CandidateAction(
            ...
            reason="MAVLink parser observes CRC/STX/sequence/signature metadata; synthetic mutation can test parser resilience without live packet attacks.",
