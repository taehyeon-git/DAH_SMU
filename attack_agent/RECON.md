...
#### dah-jammer (`EW_LINK_DEGRADATION_SIM`)
```
조건: 링크 상태 모니터링 가능
...
#### tamper (`PROTOCOL_FRAME_INTEGRITY_SIM`)
```
...
| 패턴 | 조건 | 권고 에이전트 |
|---|---|---|
| `PATROL_TRANSIT` | 속도 > 80m/s + 작전구역 내 | dah-jammer |
| `PATROL_TURNING` | 방위각 변화 > 30° | tamper |
| `LOITER_HOLDING` | 속도 < 10m/s + 위치 샘플 있음 | dah-jammer |
| `DESCENT_OR_RTL` | 고도 변화 < -50m + 속도 < 100m/s | dah-jammer |
| `OUT_OF_AREA` | 작전구역(37.85-37.96°N) 이탈 | tamper |
| `MISSION_UPLOAD_ACTIVITY` | MISSION_COUNT 수신 | dah-jammer |
...
- `--output output/stage_1_recon.json`
